...
### Инструкция к gradle задаче cleanDatabase:
Задача удаляет всех пользователей с ролью STUDENT и их решения.
...
1. SQL-check (проверка заданий на SQL-запросы).\
      ```json
   {
      "Название проверки": {
         "description": "Сообщение для случая успешного прохождения проверки",
         "score": 10,
         "test": {
            "type": "sql-check",
            "dbScript": "script.sql",
            "referenceQuery": "SELECT title\nFROM books\nWHERE author_id = (SELECT id FROM authors WHERE name = 'Стивен Кинг') AND publication_year BETWEEN 1970 AND 1980;"
         },
         "message": "Сообщение для случая неуспешного прохождения проверки",
         "special_marker": "NULL"
      },
      "Следующая проверка": {
      }
   ...
   Поле `test` содержит информацию о типе проверки (`type`), названии инициализирующего скрипта для БД (`dbScript`), а такде эталонный запрос преподавателя (`referenceQuery`).
   Скрипт можно прикрепить в форме создания задачи. \
   Поле `special_marker` указывает на то, является ли указанная проверка специальной.\
   
   Возможные типы специальных проверок: BEFORE_EACH, BEFORE_ALL, AFTER_EACH, AFTER_ALL.
   Для случая, когда проверка не является специальной, поле `special_marker` заполняется как NULL.

2. Console-check (консольная проверка).\
   ...
   "Некорректный ввод": {
    ...
    ```
   Поле `score` содержит баллы за успешно выполненную проверку.\
   Поле `test` содержит команду для выполнения (`command`), ожидаемый результат команды (`expected`).\
   Поле `special_marker` указывает на то, является ли указанная проверка специальной.\
